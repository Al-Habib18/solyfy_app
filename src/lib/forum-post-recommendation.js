...
function consineSimilarity(a, b) {
    let dotProduct = 0;
    let normA = 0;
    let normB = 0;
    for (let key in a) {
...
async function getWeightedRecommendations(userId, limit = 100, category) {
    ...
    const targetPostIds = targetActivity.map((activity) =>
        activity.post_id.toString()
    );
    const similarPosts = [];
    for (let i = 0; i < topUsers.length; i++) {
        const userActivity = await UserActivity.find({ user_id: topUsers[i] });
        userActivity.forEach((activity) => {
            const postId = activity.post_id.toString();
            if (
                !targetPostIds.includes(postId) &&
                postCounts.hasOwnProperty(postId)
            ) {
                similarPosts.push({
                    post_id: postId,
                    weight: postCounts[postId],
                });
            }
        });
    }
    ...
    const recommendations = await ForumPost.find({
        _id: { $in: topPosts },
        category,
    }).sort({ createdAt: -1 });
